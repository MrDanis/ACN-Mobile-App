...
class RNAzureCommunicationUICalling: RCTEventEmitter {
    ...
    private var timer: Timer?  // Timer to track the call duration
    private var startTime: Date?  // Start time of the call
    ...
    private func _startCallComposite(displayName: String?,
        ...
        var mode: LeaveCallConfirmationMode = LeaveCallConfirmationMode.alwaysDisabled
        ...
        headerViewData = CallScreenHeaderViewData(title: "Custom title",
                                                      subtitle: "")
        callScreenOptions = CallScreenOptions(controlBarOptions: callScreenControlBarOptions,headerViewData: headerViewData)
...
//            setupViewData = SetupScreenViewData(title: title, subtitle: subtitle)
          setupViewData = SetupScreenViewData(title: "", subtitle: "")
        }
 
      localOptions = LocalOptions(participantViewData: participantViewData, setupScreenViewData: setupViewData,cameraOn: true, microphoneOn: true, skipSetupScreen:true)
  ...
  // New code block for timer(START) 
  private func startTimer() {
        startTime = Date() // Set the start time when the call starts
    ...
    private func stopTimer() {
        timer?.invalidate()  // Invalidate the timer when the call ends
        timer = nil
    }

    @objc private func updateCallDuration() {
        ...
        // Update the header with the call duration
        headerViewData?.title = "Call Duration: \(durationString)"
        headerViewData?.subtitle = ""

    }
 //   timer code block(END)
