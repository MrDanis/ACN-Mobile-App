...
public class RNAzureCommunicationUICalling extends ReactContextBaseJavaModule {
        ...
        try {
            ...
            if (remoteAvatarImageResource != null) {
                ...
                callComposite.addOnRemoteParticipantJoinedEventHandler((remoteParticipantJoinedEvent) -> {
                    for (CommunicationIdentifier identifier : remoteParticipantJoinedEvent.getIdentifiers()) {
                        String customName = "Care Manager";
                        callComposite.setRemoteParticipantViewData(identifier,new CallCompositeParticipantViewData().setDisplayName(customName).setAvatarBitmap(remoteAvatarImageBitmap));
                    }
                });
            }
            ...
            callComposite.addOnRemoteParticipantJoinedEventHandler((event) -> {
                ...
                if (event.getIdentifiers().size() >= 1) {
                    // headerViewData.setTitle("Participant are : " + event.getIdentifiers().size());
            ...
            callComposite.addOnRemoteParticipantLeftEventHandler((event) -> {
                ...
                if (event.getIdentifiers().size() >= 1) {
                    ...
                    headerViewData.setTitle("Custom title :: waiting for others to join"+ event.getIdentifiers().size());
            ...
            callComposite.addOnCallStateChangedEventHandler((callStateChangedEvent) -> {
                ...
                startCallDurationTimer(headerViewData);
            ...
            if (URLUtil.isValidUrl(meetingInput.trim())) {
                ...
                CallCompositeLocalOptions localOptions = new CallCompositeLocalOptions().setSkipSetupScreen(true);
    ...
    private boolean isRoomsID(String meetingInput) {
        ...
        private void startCallDurationTimer(CallCompositeCallScreenHeaderViewData headerViewData) {
            callStartTime = System.currentTimeMillis(); // Store the start time
            updateCallDuration = new Runnable() {
                @Override
                public void run() {
                    ...
                    headerViewData.setSubtitle(durationText); // Update the subtitle with duration
                    handler.postDelayed(this, 1000); // Update every second
                }
            };
            handler.post(updateCallDuration); // Start updating the call duration
        }
    
        // Method to stop the timer
        private void stopCallDurationTimer() {
            if (updateCallDuration != null) {
                handler.removeCallbacks(updateCallDuration); // Stop updating
            }
        }
